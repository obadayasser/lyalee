...
const Pageone = ({collectionName , pageTitle}) => {
...
const fetchProducts = async () => {
  try {
    ...
    if (Array.isArray(collectionName)) {
      // لو فيه أكتر من كولكشن → نجيبهم كلهم مره واحدة
      const snapshots = await Promise.all(
        collectionName.map(name => getDocs(collection(db, name)))
      );
...
const handleAddOrUpdate = async (checkout = false) => {
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      // 🟢 استدعاء Thunk من Redux لتحديث الكمية
      dispatch(
      ...
      // 🟢 استدعاء Thunk من Redux للإضافة
      dispatch(
  ...
  return (
    <>
      ...
      <Container className="my-4 one" style={{ paddingTop: "150px" }}>
        <div className="head">
          <button className="head" disabled>
            {pageTitle}
          </button>
        </div>
        <Row>
              ...
              <Accordion defaultActiveKey={["0"]} alwaysOpen>
                  ...
                  <Accordion.Body>
                    <Form.Group>
                      ...
                      <Form.Check
                        ...
                        onChange={() =>
                          setFilters({ ...filters, availability: "inStock" })
                        }
                      />
                      <Form.Check
                        type="radio"
                        ...
                        onChange={() =>
                          setFilters({ ...filters, availability: "outOfStock" })
                        }
                      />
                    </Form.Group>
                  </Accordion.Body>
                ...
                <Accordion.Item eventKey="1">
                  ...
                  <Accordion.Body>
                    <Form.Group>
                      <Form.Label>
                        النطاق السعري: {filters.priceRange[0]} -{" "}
                        {filters.priceRange[1]} ج.م
                      </Form.Label>
                      <Form.Range
                        min="0"
                        max="10000"
          ...
          <Modal.Body style={{ padding: "2rem" }}>
            <Row>
              ...
              <Col md={6}>
                ...
                <h5>اختر الوزن ✨</h5>
                <div className="d-flex flex-wrap gap-2 pb-3">
                  {["من 60 إلى 90 كيلو", "من 90 إلى 125 كيلو"].map(
                    (weight, index) => (
                      <Button
                        key={index}
                        variant={
